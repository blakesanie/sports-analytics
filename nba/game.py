...
def getTimouts(pbp):
    ...
    homeTimeouts = filled.loc[pbp['HOMEDESCRIPTION'].str.contains('Timeout').fillna(False)]
    ...
    visitorTimeouts = filled.loc[pbp['VISITORDESCRIPTION'].str.contains('Timeout').fillna(False)]
...
def getScores(game, pbp):
    ...
    allScores = out
    allScores[['minutesRemaining', 'secondsRemaining']] = allScores['PCTIMESTRING'].str.split(':', 1, expand=True)
    allScores['secondsPlayed'] = (12 * 60 * allScores['PERIOD'].astype(int)) - (allScores['secondsRemaining'].astype(int) + allScores['minutesRemaining'].astype(int) * 60).astype(int)
    ...
    out = allScores[[*teamCols, 'secondsPlayed', f"{team1} Timeout", f"{team2} Timeout"]].drop_duplicates()
