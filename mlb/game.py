...
def getPlayByPlay(gameId):
    ...
    try:
        # if play by play data is cached on filesystem, use that
        ...
        homePitching.append(
            {
                ...
                **{
                    "pitcherCount": len(data["homePitchers"]) - 1,
                    "timeStamp": timeStamp,
                },
            }
        )
        awayPitching.append(
            {
                ...
                **{
                    "pitcherCount": len(data["awayPitchers"]) - 1,
                    "timeStamp": timeStamp,
                },
            }
        )
    ...
    homePitching.index = pd.to_datetime(homePitching.index).tz_convert("US/Eastern")
    awayPitching = pd.DataFrame(awayPitching).set_index("timeStamp")
    awayPitching.index = pd.to_datetime(awayPitching.index).tz_convert("US/Eastern")
    homeBatting = pd.DataFrame(homeBatting).set_index("timeStamp")
    homeBatting.index = pd.to_datetime(homeBatting.index).tz_convert("US/Eastern")
    awayBatting = pd.DataFrame(awayBatting).set_index("timeStamp")
    awayBatting.index = pd.to_datetime(awayBatting.index).tz_convert("US/Eastern")
...
def getGame(team1, team2, date, gameIndex=0):
    out = statsapi.schedule(date=date, team=team1, opponent=team2)[gameIndex]
    print("game", out)
    return out
