...
def plotLines(dfs, title=None, xLabel=None, yLabel=None, cmap={}, legendLocation='lower right', innings=None, inningsMarkers=[]):
            ...
            if col.endswith('Label'):
                ...
                for i in range(len(df[col])):
                    if df[col][i] and not pd.isnull(df[col][i]):
                        ...
                        txt = axes.text(df.index[i], df[col.replace(
                            ' Label', '')][i], df[col][i], ha='right' if df[team][i] > df[opponent][i] else 'left', va='bottom', fontsize=9)  # backgroundcolor='#ffffffc0'
                ...
                markers = axes.plot(pitchingChanges.index, pitchingChanges[team], marker='o', c=reversecmap.get(team, 'black'), ls='none', markersize=5, alpha=0.8)
                ...
                axes.step(df.index, df[col], cmap.get(team, 'black'), where='post',
                          label=col, alpha=0.8 - i * 0.3, linewidth=4.0 - i * 2, ls='-' if i == 0 else '--')
    if xLabel:
